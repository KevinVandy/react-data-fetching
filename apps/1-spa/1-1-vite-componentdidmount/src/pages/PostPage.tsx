...
class PostPage extends Component<PostPageProps, State> {
  ...
  fetchPost = async () => {
    ...
    try {
      const fetchUrl = new URL(`http://localhost:3300/posts/${postId}`);
  ...
  fetchUser = async () => {
    ...
    try {
      const fetchUrl = new URL(`http://localhost:3300/users/${post.userId}`);
      const response = await fetch(fetchUrl.href);
  ...
  deleteComment = async (commentId: number) => {
    ...
    try {
      const deleteUrl = new URL(`http://localhost:3300/comments/${commentId}`);
  ...
  postComment = async (newComment: Omit<IComment, "id">) => {
    ...
    try {
      const postUrl = new URL(`http://localhost:3300/posts/${postId}/comments`);
