...
import { getUserPosts as getUserPostsFromServer } from "../server-functions/posts";

export const Route = createFileRoute("/users/$id")({
  loader: async ({ context: { queryClient }, params: { id } }) => {
    const userId = parseInt(id);

    // Load both user and their posts in parallel using server functions
    const [user, posts] = await Promise.all([
      getUser({ data: userId }),
      getUserPostsFromServer({ data: userId }),
    ]);

    // Set the data in the query client
    queryClient.setQueryData(userQueryOptions(userId).queryKey, user);
    queryClient.setQueryData(userPostsQueryOptions(userId).queryKey, posts);
  },
  component: UserPage,
});
